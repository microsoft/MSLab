...
#region Run from Hyper-V Host to create new VMs
    ...
    $VMsPath="E:\ws2016lab_14393.2007_SCVMM"
    ...
    foreach ($VMName in $VMNames){
            ...
            New-VM -Name $VMName -NewVHDPath "$VMsPath\$VMName\Virtual Hard Disks\$VMName.vhdx" -NewVHDSizeBytes 32GB -SwitchName $vSwitchName -Generation 2 -Path "$VMsPath"
...
#region deploy hosts
    ...
    #Create DHCP reservations for Hyper-V hosts
        ...
        foreach ($HVHost in $HVHosts){
            if (!(Get-DhcpServerv4Reservation -ComputerName $DHCPServer -ScopeId $ScopeID -ClientId ($HVHost.MACAddress).Replace(":","") | Where-Object IPAddress -eq $HVHost.IPAddress)){
