    ...
    #Create Volumes
        Foreach ($Cluster in $Clusters){
            1..$Cluster.Nodes.Count | ForEach-Object {
                New-Volume -CimSession $Cluster.Name -FileSystem CSVFS_ReFS -StoragePoolFriendlyName S2D* -Size 1TB -FriendlyName "Volume$_"
            }
        }
...
#region attempt Live Migration
    ...
    $SourceStoragePath="C:\ClusterStorage\Volume1"
    $DestinationStoragePath="C:\ClusterStorage\Volume1"
...
#region move all VMs in cluster
    $SourceClusterName="S2D-Cluster"
    $DestinationClusterName="AzSHCI-Cluster"
    $VMNames=(Get-VM -cimsession (get-clusternode -cluster $SourceClusterName).Name).Name
