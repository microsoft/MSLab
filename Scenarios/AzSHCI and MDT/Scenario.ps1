...
$files=@()
$Files+=@{Uri="https://go.microsoft.com/fwlink/?linkid=2026036" ; FileName="adksetup.exe"}
$Files+=@{Uri="https://go.microsoft.com/fwlink/?linkid=2022233" ; FileName="adkwinpesetup.exe"}
$Files+=@{Uri="https://download.microsoft.com/download/3/3/9/339BE62D-B4B8-4956-B58D-73C4685FC492/MicrosoftDeploymentToolkit_x64.msi" ; FileName="MicrosoftDeploymentToolkit_x64.msi"}
$Files+=@{Uri="https://go.microsoft.com/fwlink/?linkid=866658" ; FileName="SQL2019-SSEI-Expr.exe"}
$Files+=@{Uri="https://software-download.microsoft.com/download/pr/AzureStackHCI_17784.1068_EN-US.iso" ; FileName="AzureStackHCI_17784.1068_EN-US.iso"}
#$Files+=@{Uri="https://aka.ms/ssmsfullsetup" ; FileName="SSMS-Setup-ENU"} #SQL Management Studio
...
#add deploy info to AD Object and MDT Database
...
#allow machines to boot from PXE from DC
foreach ($HVHost in $HVHosts){
...
#replace customsettings.ini with all DB data to query (wizard output)
$content=@'
[Settings]
...
GO
'@
Invoke-Sqlcmd -ServerInstance dc\sqlexpress -Database MDTDB -Query $sqlscriptcc
