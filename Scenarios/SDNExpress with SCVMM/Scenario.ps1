...
# Verify Running as Admin
    ...
    #Define DC
    $DCvm = Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*DC"}
    #Define VMM
    $VMMvm = Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*DC"}
...
$DCvm | Stop-vm
$HostVMs = Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*$VMNames*" } 
...
Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*"} | Get-VMNetworkAdapter | ? {$_.Name -ne "internet"} | Set-VMNetworkAdapterVlan -Trunk -AllowedVlanIdList $AllowVlans -NativeVlanId $NativeVlanId
Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*"} | Set-VMNetworkAdapter -DeviceNaming On
Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*"} | Start-VM
Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*"} | Get-VMIntegrationService | Enable-VMIntegrationService

Write-host "Wait for boot" -foregroundcolor Green
start-sleep 120

Get-VM | ? {$_.Name -like "$($labconfig.Prefix)*"} | % { 
                    $_.Name
...
$HostVMs | % {
    $_| % {  Invoke-Command -VMId $_.Id -ScriptBlock {   
        Get-Disk | ?{$_.Size -eq 150GB } |
...
$VMMvm| % {  Invoke-Command -VMId $_.Id -ScriptBlock {   
    Get-Disk | ? {$_.Size -eq 150GB} |
    Where partitionstyle -eq 'raw' |
    Initialize-Disk -PartitionStyle MBR -PassThru |
...
Get-NetAdapter -Name HNV -CimSession $RouterCIM | New-NetIPAddress -IPAddress ($config.LogicalNetworks | ? {$_.Name -eq "HNVPA"}).subnets.Gateways -PrefixLength ($config.LogicalNetworks | ? {$_.Name -eq "HNVPA"}).subnets.AddressPrefix.split("/")[1] -AddressFamily IPv4 -CimSession $RouterCIM
Get-NetAdapter -Name Transit -CimSession $RouterCIM | New-NetIPAddress -IPAddress ($config.LogicalNetworks | ? {$_.Name -eq "Transit"}).subnets.Gateways -PrefixLength ($config.LogicalNetworks | ? {$_.Name -eq "Transit"}).subnets.AddressPrefix.split("/")[1] -AddressFamily IPv4 -CimSession $RouterCIM
#endregion configure router nics

#region exclude VMM IPs from dhcp
Write-host "Exclude ip range from DHCP" -ForegroundColor Green
Add-DhcpServerv4ExclusionRange -ScopeId (Get-DhcpServerv4Scope -CimSession $DCCIM).ScopeId -StartRange ($config.LogicalNetworks | ? {$_.Name -eq "Management"}).subnets.PoolStart -EndRange ($config.LogicalNetworks | ? {$_.Name -eq "Management"}).subnets.PoolEnd -CimSession $DCCIM
...
#create Host Group
if(!(Get-SCVMHostGroup | ? {$_.Name -eq $config.NCHostGroupName})){
    New-SCVMHostGroup -Name $config.NCHostGroupName
}

#Create Management network
$logicalNetworks = $config.LogicalNetworks | ? {$_.Name -eq "Management"}
...
foreach ($LogicalSwitchConfig in $LogicalSwitches.LogicalSwitches){
    ...
    $logicalSwitch = Get-SCLogicalSwitch | ? {$_.Name -eq $LogicalSwitchConfig.Name }
...
$Servers | %{ Add-SCVMHost -VMHostGroup $HostGroup -ComputerName $_ -Credential $DomainAdmin -RunAsynchronously}
start-sleep 180
...
cd "E:\SDN-master\VMM\VMM SDN Express"
...
.\VMMExpress.ps1 -ConfigurationDataFile "E:\library\Fabricconfig.psd1" -verbose
    ...
    if($Undo -eq "Y"){
        ...
        Get-SCService | ? {$_.Name -eq "Gateway Manager"} | Remove-SCService
        Get-SCServiceConfiguration | ? {$_.Name -eq "Gateway Manager"} | Remove-SCServiceConfiguration 
        Get-SCServiceTemplate | ? {$_.name -eq "Gateway Deployment service Template"} | Remove-SCServiceTemplate
        ...
        #Clean up NC
        Get-SCNetworkService -ServiceType NetworkManager | Set-SCNetworkService -RemoveLogicalNetworkVIP (Get-SCLogicalNetwork | ? {$_.Name -notlike "*management*"}) -RemoveLogicalNetworkDedicatedIP (Get-SCLogicalNetwork | ? {$_.Name -notlike "*management*"})
        Get-SCVMNetwork | ? {$_.Name -ne "management"} | Remove-SCVMNetwork
        Get-SCStaticIPAddressPool | ? {$_.Name -notlike "*management*"} | % {Get-SCIPAddress -StaticIPAddressPool $_ | Revoke-SCIPAddress }
        Get-SCStaticIPAddressPool | ? {$_.Name -notlike "*management*"} | Remove-SCStaticIPAddressPool -Force

        $portProfile = Get-SCNativeUplinkPortProfile -Name "SDN Switch Uplink"
        Set-SCNativeUplinkPortProfile -NativeUplinkPortProfile $portProfile -RemoveLogicalNetworkDefinition ($portProfile.LogicalNetworkDefinitions | ? {$_.Name -notlike "*management*"})

        Get-SCLogicalNetworkDefinition | ? {$_.Name -notlike "*management*"} | Remove-SCLogicalNetworkDefinition -Force

        Get-SCLogicalNetwork | ? {$_.Name -notlike "*management*"} | Remove-SCLogicalNetwork -Force
        ...
        get-scrunasaccount | ? {$_.name -like "NC*"} |  remove-scrunasaccount

        Get-SCCustomResource | ? {$_.SharePath -like "*EdgeDeployment.cr*"} | Remove-SCCustomResource
        Get-SCCustomResource | ? {$_.SharePath -like "*NCCertificate.cr*"} | Remove-SCCustomResource
        Get-SCCustomResource | ? {$_.SharePath -like "*NCSetup.cr*"} | Remove-SCCustomResource
        Get-SCCustomResource | ? {$_.SharePath -like "*ServerCertificate.cr*"} | Remove-SCCustomResource
        Get-SCCustomResource | ? {$_.SharePath -like "*TrustedRootCertificate.cr*"} | Remove-SCCustomResource

        Get-DnsServerResourceRecord -ZoneName $config.ManagementDomainFDQN -ComputerName $config.ManagementDomainFDQN | ? {$_.Hostname -LIke "NC*"} | Remove-DnsServerResourceRecord  -ZoneName $config.ManagementDomainFDQN -ComputerName $config.ManagementDomainFDQN -Force -Confirm:$false
...
$networkService = Get-SCNetworkService | ? {$_.Model -eq "Microsoft Network Controller"}

$fabricRoleSLB = Get-SCFabricRole -Type LoadBalancer -NetworkService $networkService

$bgpPeers = @()
$bgpPeers += New-SCNCBGPPeer -RouterName "BGPRouter" -RouterIPAddress ($config.LogicalNetworks | ? {$_.Name -eq "Transit"}).subnets.Gateways -RouterAsn 65000
$bgpRouter = New-SCNCBGPRouter -LocalASN 65530 -RouterPeers $bgpPeers
$fabricRoleSLB.ServiceVMs | % {Set-SCFabricRoleResource -FabricRoleResource $_ -NCBGPRouter $bgpRouter}

$fabricRoleGW = Get-SCFabricRole -Type Gateway -NetworkService $networkService
$bgpPeers = @()
$bgpPeers += New-SCNCBGPPeer -RouterName "BGPRouter" -RouterIPAddress ($config.LogicalNetworks | ? {$_.Name -eq "Transit"}).subnets.Gateways -RouterAsn 65000
$bgpRouter = New-SCNCBGPRouter -LocalASN 65530 -RouterPeers $bgpPeers
$fabricRoleGW.ServiceVMs | % {Set-SCFabricRoleResource -FabricRoleResource $_ -NCBGPRouter $bgpRouter}
...
$BGPRouterIP = ($config.LogicalNetworks | ? {$_.Name -eq "Transit"}).subnets.Gateways

Remove-BgpRouter -Force -Confirm:$false
...
foreach($SLBVM in $SLBVMs){
    ...
    $vm = $SLBVM.Resource
    $VMIp = $null
    $VMIp = ($vm.VirtualNetworkAdapters | ? {$_.IPv4Addresses -like "$(($config.LogicalNetworks | ? {$_.Name -eq "Transit"}).subnets.Gateways.TrimEnd(".1")).*"}).IPv4Addresses[0]
    Get-BgpPeer -CimSession $RouterCIM | ? {$_.Name -eq $vm.Name} | Remove-BgpPeer  -CimSession $RouterCIM
